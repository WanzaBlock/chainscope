...
import { Bell, Activity, Brain, Server, Shield, Layers, HelpCircle, HardDrive, Network, GitMerge, ChevronDown } from 'lucide-react';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'telemetry' | 'nodes' | 'alerts' | 'ai'>('telemetry');
  ...
  useEffect(() => {
    // 1. Load chains (merge initial + custom storage)
    const storedCustom = localStorage.getItem('chainwatch_custom_chains');
    const customChains: Chain[] = storedCustom ? JSON.parse(storedCustom) : [];
    const mergedChains = [...INITIAL_CHAINS, ...customChains];
    ...
    if (storedRules) {
      ...
      const defaultRules: AlertRule[] = [
        { id: 'rule-1', chainId: 'monad', metric: 'gasPrice', condition: 'above', value: 25, isActive: true, alertName: 'Monad Gas Congestion' },
        { id: 'rule-2', chainId: 'berachain', metric: 'gasPrice', condition: 'above', value: 100, isActive: true, alertName: 'Bera Gas Surge' },
        { id: 'rule-3', chainId: 'movement', metric: 'tps', condition: 'below', value: 80, isActive: true, alertName: 'Movement Low Throughput' },
        { id: 'rule-4', chainId: 'megaeth', metric: 'tps', condition: 'above', value: 2000, isActive: true, alertName: 'MegaETH Hyper TPS Check' }
      ];
      setAlertRules(defaultRules);
      localStorage.setItem('chainwatch_alert_rules', JSON.stringify(defaultRules));
    }
  ...
  const saveCustomChains = (updatedChains: Chain[]) => {
    const customOnly = updatedChains.filter(c => c.isCustom);
    localStorage.setItem('chainwatch_custom_chains', JSON.stringify(customOnly));
  };
      ...
      setChains((currentChains) => {
        const nextChains = currentChains.map((chain) => {
          ...
          const nextValidatorNodes = chain.validatorNodes?.map(node => {
            ...
            let nextPing = node.status === 'Online' ? Math.max(8, Math.round(node.ping + latencyNoise)) : node.ping;
            let currentBlockHeight = node.status === 'Online' ? node.blockHeight + 1 : node.blockHeight;
            return {
              ...node,
              ping: nextPing,
          ...
          const nextTimestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
          ...
          const activeChainRules = alertRules.filter(r => r.chainId === chain.id && r.isActive);
          activeChainRules.forEach(rule => {
            ...
            if (rule.metric === 'activeAddresses') checkValue = chain.activeAddresses24h;

            const triggersAlert = rule.condition === 'above' 
              ? checkValue > rule.value 
              : checkValue < rule.value;

            if (triggersAlert) {
              setAlertLogs(currentLogs => {
                const isLogged = currentLogs.some(l => l.metric === rule.metric.toUpperCase() && l.chainId === chain.id && l.timestamp === nextTimestamp);
                ...
                const newLog: AlertLog = {
                  ...
                  timestamp: nextTimestamp,
                  isRead: false,
                  severity: rule.metric === 'tps' && rule.condition === 'below' ? 'critical' : 'warning'
                };
                const updatedLogs = [newLog, ...currentLogs].slice(0, 30);
                localStorage.setItem('chainwatch_alert_logs', JSON.stringify(updatedLogs));
                return updatedLogs;
              });
            }
          });
  ...
  return (
    <div className="min-h-screen bg-[#0b111e] bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:16px_16px] text-slate-200 flex flex-col antialiased font-sans">
      ...
      <div className="bg-violet-600 text-white font-mono text-[10px] py-1.5 px-4 flex justify-between select-none font-semibold tracking-wider">
        ...
        <span>UTC CLOCK: {new Date().toISOString().replace('T', ' ').substring(0, 19)}</span>
      </div>

      {/* Main Terminal Header */}
      <header className="border-b border-slate-800 bg-[#0e1626]/90 backdrop-blur-md p-5 pb-6 sticky top-0 z-40 shadow-xs" id="app-header">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display font-extrabold text-xl text-slate-105 tracking-tight flex items-center gap-2">
                ChainWatch <span className="text-[10px] text-violet-350 bg-violet-950/60 border border-violet-800/80 px-2.5 py-0.5 rounded-full font-mono font-semibold">Early Ecosystem L1/L2</span>
              </h1>
              <p className="text-xs text-slate-400 mt-0.5 font-normal">Modern high-fidelity telemetry, adversarial risk modeling, and on-chain diagnostics node analyzer</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <div className="font-mono text-xs text-right hidden sm:block">
              <span className="block text-slate-500 text-[10px] uppercase font-semibold">Tracked Networks</span>
              <span className="font-bold text-slate-300 text-sm">{chains.length} Registered</span>
            </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 flex flex-col lg:flex-row gap-6 overflow-hidden">
        ...
        <section className="w-full lg:w-85 shrink-0 h-auto lg:h-[740px]" id="left-explorer-column">
          <ChainList
            chains={chains}
        ...
        <section className="flex-1 flex flex-col h-auto lg:h-[740px] overflow-hidden" id="right-control-column">
          {/* Tabs navigation row */}
          <div className="flex border-b border-slate-800 p-1 gap-2 bg-[#121927] rounded-xl max-w-md md:max-w-lg mb-4" id="tab-navigation-rails">
            <button
              id="tab-telemetry"
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin min-h-[480px]">
            ...
            {activeTab === 'nodes' && (
              <NodeHealth chain={selectedChain} />
            )}

            {activeTab === 'alerts' && (
              <AlertConfig
                chain={selectedChain}
                rules={alertRules.filter(r => r.chainId === selectedChainId)}
                logs={alertLogs.filter(l => l.chainId === selectedChainId)}
      ...
      {showGlossary && (
        <section id="glossary-drawer" className="bg-[#0e1626] border-t border-slate-800 p-6 md:p-8 shadow-inner animate-in slide-in-from-bottom duration-300">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs text-slate-300">
              {TECHNICAL_GUIDELINES.map((guideline, index) => (
                <div key={index} className="space-y-2 p-4 bg-[#141d2f] border border-slate-800/80 rounded-xl shadow-xs">
      ...
      <footer className="border-t border-slate-850 px-4 py-5 mt-auto bg-[#080d15] select-none text-[10px] text-slate-500 text-center font-mono">
        ChainWatch Diagnostic Terminal. All simulated metrics are dynamically generated under Cosmos-SDK, Parallel-EVM, and SVM BFT-consensus specifications.
      </footer>
    </div>
  );
}
